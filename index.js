'use strict';
var chalk, fs, gutil, httpsClient, path, paths, request, sprintf, through, util, async, mkdirp;
...
module.exports = {
  createClient: function (options) {
      ...
      req = request('GET', _this._paths.get_or_create_resources({
        ...
        res.on('data', function (data) {
          if (parseInt(res.statusCode) === 200) {
            ...
            req.emit('error', new Error(res.statusCode + ': ' + httpsClient.STATUS_CODES[res.statusCode] + ' reaching the project'));
          }
        });
        res.on('end', function () {
          try {
    ...
    this.languages = function (resource, opt, callback) {
      ...
      req = request('GET', this._paths.get_languages({
        ...
        res.on('end', function () {
          ...
          languages = Object.keys(languages).map(function (key) {
            var isoCode, customCode, langObj = {};

            if (options.custom_language_codes &&
              options.custom_language_codes[key] &&
              ...
              return options.custom_language_codes[key];
            } else {
              return key;
            }
          });
    ...
    this.pullResource = function (callback) {
      ...
      buffer = through.obj(function (file, enc, cb) {
        ...
        if (file.isBuffer()) {
          ...
          languages = _this.languages(path.basename(file.path).replace(/\./, ''), {
